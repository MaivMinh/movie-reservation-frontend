...
const Booking = () => {
    ...
    Modal.warning({
      ...
      content: (
        <div className="w-full">
          <p className="font-semibold">Bạn chỉ có thể chọn tối đa 5 ghế</p>
        </div>
      ),
    });
  };
  ...
  return (
    <div className="text-black w-[98%] mx-auto bg-[#f5f5f5af] min-h-screen grid grid-cols-10 gap-4">
      ...
      <div className="col-span-3 rounded-xl bg-white p-4 pt-6 border-t-6 border-amber-500">
        ...
        {selectedSeat.size > 0 && (
          <div className="border-dashed border-b-[0.5px] border-b-gray-800 mt-10 flex flex-col items-start justify-start gap-y-4">
            {/* Chứa danh sách ghế ngồi đã chọn */}
            ...
            <span className="text-md font-semibold text-[#f06312]">
              {selectedSeat.size > 0
                ? [...selectedMap.values()]
                    ...
                    .reduce((a, b) => a + b)
                : 0}
              <span class="inline-block font-bold text-primary ">&nbsp;₫</span>
            </span>
          </p>
        </div>
        <div>
